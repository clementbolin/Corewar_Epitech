...
void ld(cor_t *vm UNUSED, champ_t *champ, linkv_t *link)
{
    nodev_t *node = NULL;
    int value = 0;
    ...
    if (link == NULL) return;
    node = link->head;
    if (check_arg_ld(link) == 84) return;
    value = node->value;
    node = node->next;
    if (node->value > 16)
        return;
    value = champ->pc + value % IDX_MOD;
    if (value < 0 || value > MEM_SIZE) return;
    value = vm->map[value];
